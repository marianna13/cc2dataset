...
def check_ext(ext, media_ext=List[str]) -> bool:
    """Check a yt-dlp extractor"""

    if 'porn' in ext.IE_NAME.lower() or ext.IE_NAME == 'generic':
        return False
    try:
        tests = ext._TESTS
        for test in tests:
            if test['info_dict']['ext'] in media_ext:
                if test['info_dict'].get('age_limit') is None:
                    return True
        return False
    except:
        return False
...
class PlatformExtractor:
    ...
    def __init__(self, platform_type: str, platforms:list=None):
        self.platform_type = platform_type
        extractors = gen_extractors()
        
        if platforms:
            extractors = [ext for ext in extractors if ext.IE_NAME in platforms]
        else:
            media_ext = platform_extension[platform_type.split("_")[0]]
            extractors = [ext for ext in extractors if check_ext(ext, media_ext)]
        self.extractors = extractors
    
    def check_url(self, url: str) -> bool:
        """Check an arbitrary URL whether it's a platform link"""
        if url is None:
            return False
        if not url.startswith("http://") or url.startswith("https://"):
            return False
        for ext in self.extractors:
            if ext._match_valid_url(url):
                return True
        return False
...
def extract_video_platform_from_links(links, platforms):
    platform_extractor = PlatformExtractor("video_platform")
    filtered_links = [{"url": link["url"], "alt": link.get("text", "")} for link in links if platform_extractor.check_url(link.get('url'))]
    return filtered_links

def extract_audio_platform_from_links(links):
    platform_extractor = PlatformExtractor("audio_platform")
    filtered_links = [{"url": link["url"], "alt": link.get("text", "")} for link in links if platform_extractor.check_url(link.get('url'))]
    return filtered_links
...
def make_links_absolute(links, base_url):
    return [{"url": make_link_absolute(link.get("url", ""), base_url), "alt": link.get("alt")} for link in links]


def extract_documents_from_links(links, document_type, platforms):
    ...
    elif document_type == "video_platform" or platforms:
        return extract_video_platform_from_links(links, platforms)
    else:
        raise ValueError(f"Unknown document type {document_type}")


def extract_documents_from_wat(stream, document_type, platforms):
    """Extract document from stream"""
    all_links = []
    try:
        for record in tqdm(ArchiveIterator(stream, record_types=WarcRecordType.metadata, parse_http=False)):
            try:
            ...
            filtered_links = extract_documents_from_links(links, document_type, platforms)
...
def process_wat(path, document_type, platforms):
    ...
    with fsspec.open(path, "rb") as f:
        ...
        for e in extract_documents_from_wat(tf, document_type, platforms):
...
def read_wat_index_files(shard_count, wat_count, source_cc_protocol, num_threads):
...
def deduplicate_repartition_count(df, output_path, wat_count, spark, shuffle=False):
    ...
    repartitioned = uniques.repartition(max(1, wat_count // 500))
...
def process_one_part(output_path, wat_index_files, build_spark, shuffle, document_type, source_cc_protocol, platforms):
    """Process one part"""
    spark = build_spark()
    ...
    def extract(x):
        x = list(x)
        yield from process_wat(prefix + x[0], document_type, platforms)
...
def process_multi_part(
    output_path, wat_index_files, build_spark, multipart, shuffle, resume, document_type, source_cc_protocol, platforms
):
    """Process multi part"""
    if resume:
    ...
    for i in range(start_part, multipart):
        ...
        process_one_part(part_path, wat_index_files[start:end], build_spark, False, document_type, source_cc_protocol, platforms)
...
def cc2dataset(
    ...
    if resume is None:
        wat_index_files = read_wat_index_files(wat_index_count, wat_count, source_cc_protocol, num_threads)
    ...
    if multipart is None:
        process_one_part(output_path, wat_index_files, build_spark, shuffle, document_type, source_cc_protocol, platforms)
    else:
        process_multi_part(
            output_path, wat_index_files, build_spark, multipart, shuffle, resume, document_type, source_cc_protocol, platforms
        )
